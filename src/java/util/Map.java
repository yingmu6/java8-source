...
/**
 * An object that maps keys to values（将键与值映射的对象）.  A map cannot contain duplicate keys（map不能包含重复的键）;
...
public interface Map<K,V> {
    ...
    /**
     * Returns a {@link Set} view of the mappings contained in this map. //返回当前Map对应的Set集合视图
     * The set is backed by the map, so changes to the map are
     * reflected in the set, and vice-versa.  If the map is modified
     * while an iteration over the set is in progress (except through
    ...
    /**
     * A map entry (key-value pair).//map的条目：包含键值对  The <tt>Map.entrySet</tt> method returns
     * a collection-view of the map（entrySet返回map的集合视图）, whose elements are of this class.  The
     * <i>only</i> way to obtain a reference to a map entry is from the
     * iterator of this collection-view.  These <tt>Map.Entry</tt> objects are
     * valid <i>only</i> for the duration of the iteration; more formally, //Map.Entry：仅在迭代期间有效
    ...
    interface Entry<K,V> {
        ...
        /**
         * Returns the key corresponding（相应的） to this entry. //返回entry相应的key
         *
        ...
        /**
         * Returns a comparator that compares {@link Map.Entry} in natural order on key. (返回按key自然排序的Map.Entry对应的比较器comparator)
         *
        ...
        /**
         * Returns a comparator that compares {@link Map.Entry} in natural order on value（按值value自然排序）.
         *
         * <p>The returned comparator is serializable and throws {@link
     ...
     * @since 1.8
     */
    default void forEach(BiConsumer<? super K, ? super V> action) { //遍历Map中的所有条目，依次执行给定的操作
    ...
    default V computeIfAbsent(K key,
        ...
        V v;
        if ((v = get(key)) == null) { //若key对应的值存在，则直接返回，没有则进行创建
            V newValue;
            if ((newValue = mappingFunction.apply(key)) != null) { //通过函数式接口去创建值，若值不为空，就设置到Map中
                put(key, newValue);
                return newValue;
            }
        }
