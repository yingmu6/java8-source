...
public class Hashtable<K,V>
    ...
    public synchronized V put(K key, V value) {
        // Make sure the value is not null（key值是非null的）
        if (value == null) {
            throw new NullPointerException();
        }
        ...
        Entry<?,?> tab[] = table;
        int hash = key.hashCode();
        int index = (hash & 0x7FFFFFFF) % tab.length; //用key的hash值取模，计算出数组下标
        @SuppressWarnings("unchecked")
     ...
     */
    private static class Entry<K,V> implements Map.Entry<K,V> { //Entry:条目，入口
        final int hash;
        final K key;
        V value;
