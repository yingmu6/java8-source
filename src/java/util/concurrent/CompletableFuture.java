 ...
 * manipulating（操作） status and results, CompletableFuture implements
...
public class CompletableFuture<T> implements Future<T>, CompletionStage<T> {
    /**
     * @csy-01-29 异步执行待了解
     * 解：使用Future获得异步执行结果时，要么调用阻塞方法get()，要么轮询看isDone()是否为true，
     * 这两种方法都不是很好，因为主线程也会被迫等待。从Java 8开始引入了CompletableFuture，它针对Future做了改进，
    ...
    volatile Object result;       // Either the result or boxed AltResult （执行的结果，有异常时result为null）
    ...
    final boolean completeValue(T t) { //使用的CAS确保同步安全？
        return UNSAFE.compareAndSwapObject(this, RESULT, null,
                                           (t == null) ? NIL : t);
    }
    ...
    /**
     * Creates a new incomplete（未完成的） CompletableFuture.
     */
     ...
     */
    private CompletableFuture(Object r) { //创建给定
        this.result = r;
    }
     ...
     */
    public static <U> CompletableFuture<U> completedFuture(U value) { //构建带有指定值的CompletableFuture
    ...
    public T get() throws InterruptedException, ExecutionException {
        Object r;
        return reportGet((r = result) == null ? waitingGet(true) : r); //若获取的结果为null，则等待直到获取到结果
    }
