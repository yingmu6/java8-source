 ...
 * properties（能够持久化属性值）. The {@code Properties} can be saved to a stream
 * or loaded from a stream（可以把属性值保存到流中，也可以从流程加载属性值）. Each key and its corresponding(相应地) value in
 ...
 * <p>
 * Because {@code Properties} inherits（继承） from {@code Hashtable}, the
 * {@code put} and {@code putAll} methods can be applied to a
 * {@code Properties} object.  Their use is strongly discouraged（强烈制止） as they
 * allow the caller to insert entries whose keys or values are not
 * {@code Strings}.  The {@code setProperty} method should be used
 * instead（使用setProperty方法，而不要使用put方法）.  If the {@code store} or {@code save} method is called
 ...
 * Characters that cannot be directly represented（代表） in this encoding can be written using
 ...
 */
public
class Properties extends Hashtable<Object,Object> { //Properties继承于Hashtable。表示一个持久的属性集。属性列表中每个键及其对应值都是一个字符串
    /**
     * use serialVersionUID from JDK 1.1.X for interoperability（ˌɪntərˌɒpərəˈbɪləti 互通性）
     */
     ...
     * @serial
     */
    protected Properties defaults; //持有的默认属性值
     ...
     */
    public Properties(Properties defaults) { //可以指定默认的属性值
    ...
    public synchronized Object setProperty(String key, String value) {
        return put(key, value); //使用HashMap的方法设置值
    }
            ...
            while (true) {
                ...
                if (c != '\n' && c != '\r') {
                    ...
                    if (len == lineBuf.length) {
                        int newLength = lineBuf.length * 2; //扩容处理
                        if (newLength < 0) {
                            newLength = Integer.MAX_VALUE;
                        }
                        char[] buf = new char[newLength];
                        System.arraycopy(lineBuf, 0, buf, 0, lineBuf.length); //使用数组拷贝
                        lineBuf = buf;
                    }
