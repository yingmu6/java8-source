 ...
 * class, such as <code>Hashtable</code>, which maps keys to values.（Hashtable是Dictionary的实现类）
 * Every key and every value is an object. In any one <tt>Dictionary</tt>
 * object, every key is associated with at most one value（最多与一个值关联）. Given a
 * <tt>Dictionary</tt> and a key, the associated element can be looked up（查出）.
 * Any non-<code>null</code> object can be used as a key and as a value.（Dictionary的key与value都是非null的）
 * <p>
 * As a rule（通常）, the <code>equals</code> method should be used by
 * implementations of this class to decide if two keys are the same.（子类需要重写equal()方法来判断两个key是否相等）
 * <p>
