...
public class HashMap<K,V> extends AbstractMap<K,V>
    ...
    static class Node<K,V> implements Map.Entry<K,V> {
        final int hash;
        final K key; //key为null时，hash值为0
        V value;
        Node<K,V> next;

        Node(int hash, K key, V value, Node<K,V> next) { //每个hash值与key、value、下一个节点关联
            this.hash = hash;
            this.key = key;
            this.value = value;
            this.next = next;
     ...
     */
    static final int hash(Object key) { //获取指定key对应的hash值
        int h;
        return (key == null) ? 0 : (h = key.hashCode()) ^ (h >>> 16); //key为空，hash值设置为0，否者使用hash函数取值
    }
     ...
     */
    transient Set<Map.Entry<K,V>> entrySet; //key、value对应的集合视图
    ...
    int threshold; //极限、临界值（等于capacity * load factor）
     ...
     * value is replaced.（如果已经存在key了，就会替换对应的值）
     *
     ...
     * @param onlyIfAbsent if true, don't change existing value（如果设置为true，不改变已经存在key的值，absent：不存在的；缺席的）
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
        ...
        if ((p = tab[i = (n - 1) & hash]) == null)
            tab[i] = newNode(hash, key, value, null); //新建数据节点
        else {
            Node<K,V> e; K k;
            if (p.hash == hash &&
                ((k = p.key) == key || (key != null && key.equals(k)))) //用eqauls方法判断key是否存在
                e = p;
        ...
        ++modCount;
        if (++size > threshold) //若添加元素后的数量大于阈值值，则重新调整大小
            resize();
     ...
     * @return the table
     */
    final Node<K,V>[] resize() { //重新计算size的值
    ...
    @Override
    public V putIfAbsent(K key, V value) { //只在key不存在时设置值
