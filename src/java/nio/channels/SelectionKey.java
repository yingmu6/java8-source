 ...
 * selector（每当通道被选择器注册时，都会创建一个选择键）.  A key remains valid until it is <i>cancelled</i> by invoking its
 * {@link #cancel cancel} method（在调用cancel方法前，key都是有效的）, by closing its channel, or by closing its
 * selector.  Cancelling a key does not immediately remove it from its
 * selector; it is instead（代替） added to the selector's <a
...
public abstract class SelectionKey {
     ...
     */
    public abstract SelectableChannel channel(); //返回key关联的通道
     ...
     */
    public abstract Selector selector(); //获取选择器
