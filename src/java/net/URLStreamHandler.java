...
// FIX: should not assume query if opaque
        ...
        if (start < limit) {
            int queryStart = spec.indexOf('?'); //解析出查询信息
        ...
        if (!isUNCName && (start <= limit - 2) && (spec.charAt(start) == '/') &&
            ...
            int ind = authority.indexOf('@'); //解析出用户信息
            if (ind != -1) {
        ...
        setURL(u, protocol, host, port, authority, userInfo, path, query, ref); //构建新的URL
    }
