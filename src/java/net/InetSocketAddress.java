...
/**
 *
 * This class implements an IP Socket Address (IP address + port number) （Socket地址包含Ip地址和端口号）
 * It can also be a pair (hostname + port number), in which case an attempt
 * will be made to resolve the hostname. If resolution fails then the address
 * is said to be <I>unresolved</I> but can still be used on some circumstances（情况）
    ...
    extends SocketAddress
{
    ...
    private static class InetSocketAddressHolder {
        ...
        public final boolean equals(Object obj) {
            ...
            boolean sameIP;
            if (addr != null) //IP地址不为空时，比较IP地址
                sameIP = addr.equals(that.addr);
            else if (hostname != null) //主机号不为空时，比较主机号
     ...
     *                  parameter is <TT>null</TT>. (port超出范围或hostname为空，都会抛出异常)
     * @see     #isUnresolved()
     * @return  a {@code InetSocketAddress} representing the unresolved
     *          socket address
     * @since 1.5
     */
    public static InetSocketAddress createUnresolved(String host, int port) { //创建一个未解析的socket地址（不会对hostname进行解析）
