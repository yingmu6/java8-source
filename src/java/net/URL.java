...
public final class URL implements java.io.Serializable {
     ...
     */
    public URL(String spec) throws MalformedURLException { //从指定的字符串，创建URL对象
        this(null, spec);
    }
        ...
        throws MalformedURLException
    {
        ...
        try {
            ...
            while ((limit > 0) && (spec.charAt(limit - 1) <= ' ')) {
                limit--;        //eliminate trailing whitespace（剔除尾部空格）
            }
            while ((start < limit) && (spec.charAt(start) <= ' ')) {
                start++;        // eliminate leading whitespace（剔除首部空格）
            }
            ...
            for (i = start ; !aRef && (i < limit) &&
                ...
                if (c == ':') {
                    ...
                    String s = spec.substring(start, i).toLowerCase(); //取":"前的内容作为协议
                    if (isValidProtocol(s)) { //校验协议是否正确
                        newProtocol = s;
                        start = i + 1;
                    }
                    break;
            ...
            handler.parseURL(this, spec, start, limit); //解析URL字符串
     ...
     * （URLConnection实例在创建时不会建立实际的网络连接，也就是openConnection()方法获取到实例时还没有建立连接，只有调用connect()方法时才会建立连接）
     *
