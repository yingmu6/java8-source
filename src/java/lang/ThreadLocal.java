...
public class ThreadLocal<T> {
     ...
     * less common cases.
     */
    private final int threadLocalHashCode = nextHashCode(); //使用hash table以及hash值查找
     ...
     * maintaining thread local values（用来维护线程局部变量）. No operations are exported
    ...
    static class ThreadLocalMap {
         ...
         */
        static class Entry extends WeakReference<ThreadLocal<?>> { //维护线程局部变量
        ...
        private Entry getEntry(ThreadLocal<?> key) {
            int i = key.threadLocalHashCode & (table.length - 1); //通过hash值计算出索引值
            Entry e = table[i];
            if (e != null && e.get() == key)
                return e;
            else
