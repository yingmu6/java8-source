 ...
 * of a digital signature algorithm（数字签名算法）. Digital signatures are used for
 * authentication and integrity assurance of digital data.
 * （数字签名用于数字数据的身份验证和完整性保证）
 *
 ...
 * {@code SHA256withRSA}.
 * The algorithm name must be specified（算法名必须要指定）, as there is no default.
 *
 * <p> A Signature object can be used to generate and verify digital
 * signatures.
 *
 * <p> There are three phases（三个阶段） to the use of a Signature object for
 * either signing data or verifying a signature:<ol>
 *
 * <li>Initialization（初始化阶段）, with either
 *
 *     <ul>
 *
