...
/**
 * <code>ScriptEngine</code> is the fundamental（基本的） interface whose methods must be
 * fully functional in every implementation of this specification.
 * <br><br>
 * These methods provide basic scripting functionality（功能）.  Applications written to this
 * simple interface are expected to work with minimal（最小的） modifications in every implementation.
...
public interface ScriptEngine  { //脚本引擎

    /**
     * Reserved（保留的） key for a named value that passes
     ...
     */
    public Object eval(String script, ScriptContext context) throws ScriptException; //执行指定的脚本
