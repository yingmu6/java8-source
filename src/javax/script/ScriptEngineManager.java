...
/**
 * The <code>ScriptEngineManager</code> implements a discovery（发现） and instantiation（实例化）
 * mechanism（机件） for <code>ScriptEngine</code> classes and also maintains a
 ...
 * as well as utility（有用的） methods which look up factories on the basis of language name, file extension
 * and mime type.
 * <p>
 * The <code>Bindings</code> of key/value pairs, referred to as the "Global Scope"  maintained（保持、维持、维修）
...
public class ScriptEngineManager  {
    ...
    private void init(final ClassLoader loader) { //初始化操作
    ...
    private void initEngines(final ClassLoader loader) { //初始化引擎
        Iterator<ScriptEngineFactory> itr = null;
        try {
            ServiceLoader<ScriptEngineFactory> sl = AccessController.doPrivileged( //通过java SPI方式获取到引擎工厂列表
